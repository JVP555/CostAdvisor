...
- 🟢 **Scrum 70 (part 1)** — Index projection service (per price series, vintaged run). Named dependencies "the forecast storage subtask" and "W3-A" (series layer) do not exist as separate tickets in this repo — built minimally inline: `(commodity_id, region)` is the series grain directly off `IndexValue` (no new `PriceSeries` entity), and forecasts live in a new pair of tables `IndexValue`'s own queries never touch (so the Scrum 22 backward buy-window signal is unaffected by construction, not by an added filter)
  - 🟢 `IndexProjectionRun` / `IndexProjectionPoint` models + migration `ip1a2b3c4d5e` (no RLS — platform-level, mirrors `commodity_indexes`/`index_values`); applied clean, downgrade/upgrade cycle verified
  ...
  - 🟢 Tests `tests/test_index_projection.py` (10, all passing): fitted/hold/no-history/insufficient-points, re-run creates a new vintage and leaves the first intact, fitted output pinned against a hand-computed OLS fit, endpoint permission gates
  - 🟢 **Verified live**: local dev DB access was initially blocked by a decoy — a freshly-provisioned native Windows Postgres 18 service was also listening on `localhost:5432` with no `costadvisor` role, while the actual project DB is WSL Ubuntu's own Postgres 16 (`127.0.0.1:5432` inside WSL, not forwarded to Windows). Ran migration + full test suite from inside WSL using the already-checked-in `backend/venv` (which is WSL-native, not broken as previously assumed from a native-Windows shell) — **full suite 218 passed**, including all 21 Scrum 70 tests
- 🟢 **Scrum 70 (part 2)** — Lock/hold decision off the forward should-cost. Forward should-cost is computed **generically** off each cost model's own `FormulaVersion` (works for hand-built and catalog-linked models alike) rather than requiring a Wave-2 `FormulaRegionCoverage` catalog link
  ...
  - 🟢 Regression test `test_buy_windows_unaffected_by_forecast_storage` — proves forecast storage never perturbs the existing backward signal, because forecasts never land in `index_values`. **Passed live.**
  - 🟢 Tests `tests/test_buy_windows.py` (+7, all passing — 11 total in file): insufficient-without-projection, horizon/vintage/method echoed, verdict flips across two vintages (upward → `lock`, downward → `hold`), all-fixed-formula → `neutral`, permission gates
