...
- 🔴 **Scrum 16** — Self-serve onboarding (empty states, example data, guidance to first should-cost vs actual gap)
  ...
  - 🔴 "Load example data" works for a brand-new team and produces a runnable should-cost (in-app button still 🔴; the CLI/deploy path is done — see `seed_all.py` below)
  - 🟢 **`backend/seed_all.py`** — one idempotent command (`cd backend && alembic upgrade head && python seed_all.py`; on Railway `cd /app && python seed_all.py`) that fully populates a fresh DB in dependency order: (1) platform reference data via `app.seed.seed()`+`seed_update()` → (2) real users/teams/memberships inline via ORM (5 users / 5 teams / 9 memberships mirrored from dev, `test-*` fixtures excluded; `jil@staminachem.com`=`13099867…` + team `6ee41dc2…` seeded first because stages 3–4 resolve them by id; `Plan` resolved by name "Free" so a per-DB plan UUID can't break the FK) → (3) `seed_shadow_library.run()` (+29 indexes, 42 platform templates) → (4) `seed_staminachem.run()` (demo products/suppliers/cost-models/prices/volumes). Uses `bypass_rls_var.set(True)` (contextvar form, re-applied per transaction). Verified idempotent (re-run = `+0` created across all stages). Present on `dev` + `dev-push` (deploy needs it); the docs are stripped from `dev-push`
  - 🟢 **Dev tooling (same session):** `AGENTS.md` at repo root — compact agent quick-start (run/tests, the `dev`→`dev-push` rebuild-vs-merge deploy model, common mistakes); a root convention file (like `CLAUDE.md`), **stripped from `dev-push`**. `AuthContext.jsx` logout no longer hardcodes `http://localhost:3333`: on `dev` → relative `/login`; on `dev-push` the deploy branch keeps its own `VITE_LANDING_URL || https://www.costadvisor.org` landing redirect (intentional branch divergence, preserved through the merge)
...
- 🟢 Tests `tests/test_seed_combos.py` (14): parser current/shifted/container-less markup + problems, tier-count mismatch, weight tolerance, unknown code + duplicate, chain cycle, E2E completeness + idempotency, CONF-LOW + correction-log attachment, sample should-costs (plain/chained/fallback), API-replace-preserves-seeded-lines, regions idempotent. Full suite 112 passed
...
- 🟢 Tests: `test_weighted_formulas.py` now 20 — evaluation math (index +10% on a 60% line → 106/1060; exact anchor reproduction at base), catalog-style Σ=110 rebasing, evaluable-state reasons + data gaps, review endpoint, price upload (dry-run/apply/errors/403). Full suite 118 passed; frontend build passes
...
- 🔴 **Next scrum — index values for catalog commodities (FD-1 scoped: free feeds first)**: map the 25 `retrieval_status=free` commodities' `free_source_name` (World Bank Pink Sheet, EIA, Eurostat, FRED…) onto the scraper registry with per-series IDs, write quarterly `index_values` at the Scrum-56 region grain, live-scrape verification per feed; proxy estimation (executing `proxy_logic`) stays SCRUM-67/80. Also still open: real price data for the 676 anchors (business/data)
