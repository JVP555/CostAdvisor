...
- 🟢 **Scrum 21** — Predictive index forecasting (directional, uncertainty-honest)
  ...
  - 🟡 **Intelligence page (`/intelligence`, new navbar item)** — product-centric "market & pricing intelligence" (modelled on `sample_idea/intelligence_mockup.html`). Landing: family-grouped product cards → per-card should-cost `Sparkline` + trend, lazy-loaded via IntersectionObserver from `/api/costing/evolution`. Detail `/intelligence/:costModelId` (one `/api/costing/brief` call), Tab 1 "Market & Pricing" wired read-only: should-cost **index** history (rebased base 100), index-component/driver decomposition table, stored (Redis-cached) AI narrative + forward signals, derived cycle-position + snapshot cards. Tab 2 "Product Intelligence" is a **flagged placeholder**. No backend change.
    - 🟢 **Fix: the `FORECAST_BAND = 0.015` stub survived here even after Scrum 64 removed its twin from `ForecastArea.jsx`** — a deeper re-verification against the original ticket text (`sample_idea/scrum21/prompt.txt`, which explicitly says this placeholder "should not reappear here in any form") caught it in `IntelligenceDetailArea.jsx` and it's now removed: no more dashed forecast band, `splitIndex`/`splitLabel`, or the `FORECAST_STEPS`/`nextQuarters` helpers it needed — same treatment as `ForecastArea.jsx` (real history only, caption points at the Index Library / buy-window verdict for a real forecast). This page's should-cost index is a per-formula weighted blend, not a single `(commodity, region)` series, so it still has no 1:1 real-forecast substitute without a page redesign — removing the fake one was the correct, minimal fix rather than fabricating a replacement.
...
- 🔴 **Scrum 33** — Multi-source index validation (cross-check values, flag anomalies)
  ...
  - 🔴 **Redefined + reconfirmed blocked (source: `sample_idea/scrum34/prompt.txt`, "Index data-quality validation over the resolution layer")** — the fuller ticket text drops the original "compare two live sources" framing (we mostly don't hold two independent readings of one series) in favor of surfacing *declared and structural* contradictions already sitting in a foundational drop: a delivered `_issues.csv` defect list to load (not re-derive), `combo_lines.proxy_status` disagreeing with its own `type_codes` row, `type_codes.resolution="ambiguous"` as a first-class state, and card-vs-series disagreement on agency/unit/incoterm (`index_feeds.csv`/`index_commodities.csv`). **Confirmed absent from this repo** (search for `type_codes`/`combo_lines`/`index_feeds.csv`/`index_commodities.csv`/`_issues.csv` — zero hits outside this ticket and CLAUDE.md's own prose) — the ticket's own text says *"This blocks on DB-5. Do not start it first,"* and the user confirmed skipping it rather than building a stand-in against our own schema. The two named "load-time failure" prep fixes (`CommodityIndex.provider` widened past `String(64)`, `FREQUENCIES` vocab broadened) are real, cheap, and un-blocked whenever picked up — but pointless alone without the drop to load

- 🟢 **Scrum 33b** — Cost-structure estimator for catalog combos with no usable decomposition. Unrelated to Scrum 33 above (multi-source index validation) despite the adjacent number — this is the ticket CLAUDE.md's own backlog notes already (mis-)cross-reference as "SCRUM-74 (Scrum 32 estimator — cost-structure proposal service)"; recorded as "33b" here, same collision-avoidance pattern as Scrums 27b/28b/30b/31b
  - 🟢 **Everything the ticket names as existing infrastructure turns out not to exist in this repo** (confirmed by search): no `docs/CONTENT_MODEL.md`, no real `ai_draft`/`imported`/`human_approved` provenance triad, no "synthesis routes" feedstock-label dataset. The one genuinely real, existing piece — `FormulaRegionCoverage.needs_review`/`reviewed_by`/`reviewed_at` (Scrum 60's review workflow) — is **extended**, not forked: a new orthogonal `provenance` column (`"imported"` default / `"ai_draft"` / `"human_approved"`)
  - 🟢 **Primary evidence source (confirmed with the user): sibling-region recipe inheritance** — for a combo with no trustworthy lines, `_find_sibling_recipe` looks at the same `FormulaTemplate`'s *other* regions that already carry a `CONF-HIGH`/`CONF-MED` human recipe (never a `CONF-LOW` sibling — that would propagate one bad guess into a second region), inherits the same relative weight proportions, and re-checks real series availability specifically in the *target* region (`_has_usable_series`) — an unavailable candidate is **flagged** (`series_available=False`), never silently dropped
  - 🟢 **Secondary/fallback evidence: priced-history correlation** — `_linked_price_history` (a `CostModel` whose `FormulaVersion.source_coverage_id` points at the exact combo, ≥4 quarters of `ActualPrice`) confirms/adjusts a sibling-derived proposal when real linked history exists, and is the *sole* fallback (deterministic Pearson correlation via `_correlate_candidates`, restricted to `feedstock`/`energy`-role indexes, `|r| ≥ 0.3`) when a template has no sibling regions at all — top-2 candidates weighted by r² up to 70%, remainder as an explicit `component_type="fixed"` "Margin / unexplained" line (margin stays a line inside the 100% total, never additive on top)
  - 🟢 Neither evidence source available → `{"evaluable": false, "reason": ...}`, a first-class tested outcome — never a fabricated full-looking recipe
  - 🟢 **Draft-then-approve, never mutates the live recipe until approved** — new `EstimatorProposal`/`EstimatorProposalLine` staging tables (migration `es1a2b3c4d5e`, RLS transitive through the parent template, same shape as `formula_template_components`/`formula_region_coverage`). `create_or_update_proposal` upserts by `(template_id, region)` (re-running never duplicates; no-ops against an already-`human_approved` combo). `approve_proposal` is the *only* function that ever writes real `FormulaTemplateComponent` rows + flips `needs_review=False`/`provenance="human_approved"` on the coverage — this is what makes the combo priceable end to end via `evaluate_weighted_template`
  - 🟢 `POST /{template_id}/estimator/propose?region=`, `GET /{template_id}/estimator/proposal?region=`, `POST /estimator/proposals/{id}/approve`, `POST /estimator/proposals/{id}/reject` (all gated via the existing `_require_template_edit` platform/team-fork rule) + `GET /estimator/backtest?template_id=` (platform `formulas.edit`) — a non-circular-by-construction report (sibling search always excludes a combo's own region, so a trustworthy combo's own real lines are never their own evidence) with an optional `template_id` scope for a fast, targeted run instead of the whole catalog
  - 🟢 No frontend — the ticket's own explicit scope: "Not a screen"
  - 🟢 Tests `tests/test_formula_estimator.py` (7): sibling inheritance + candidate_reason naming the source region, series-unavailable flagging on an inherited line, no-evidence → evaluable=false, blind correlation with a linked-history fixture (weights sum to 100 incl. the explicit margin line), never-mutates-until-approved + re-run upsert/no-dup/no-op-on-approved + approve makes the combo priceable via `evaluate_weighted_template` + double-approve 400, backtest match-fraction on a held-out combo with non-circularity proven by removing the sibling, permission gates. Full suite 297 passed, verified live (migration + downgrade/upgrade cycle) from inside WSL
...
- 🔴 **SCRUM-73 (Scrum 33 — multi-source index validation, redefined)** — reframed from "compare two live sources" to "surface declared/structural contradictions already in the drop" (`_issues.csv`, `proxy_status` self-contradictions, `ambiguous` resolution states, agency/unit/incoterm mismatches). **Hard-blocked** on the Index Data Layer v2 resolution layer (SCRUM-74/DB-5) which isn't in this repo; two schema constraints (`CommodityIndex.provider` `String(64)` too short, `FREQUENCIES` vocab missing values the drop uses) are called out as load-time failures to fix as part of this work, whenever it starts. **Reconfirmed** (source: `sample_idea/scrum34/prompt.txt`, same full ticket text word-for-word, including its own explicit "do not start it first") — re-searched the repo, still zero trace of `type_codes`/`combo_lines`/`index_feeds.csv`/`index_commodities.csv`/`_issues.csv` anywhere; asked the user rather than building a stand-in against our own schema, and skipping was the chosen call. Genuinely nothing to build here until DB-5 lands.
...
- 🟢 **SCRUM-74 (Scrum 33b estimator — cost-structure proposal service)** — shipped (see "Scrum 33b" above for detail). This note originally mislabeled the ticket as "Scrum 32" — the real ticket text lives at `sample_idea/scrum33/prompt.txt`, filed above as "Scrum 33b" to avoid colliding with CLAUDE.md's pre-existing "Scrum 33" (multi-source index validation) entry. The named blocker (no persisted `ai_draft`/`imported`/`human_approved` review/provenance layer) is resolved by the new `provenance` column + `EstimatorProposal`/`EstimatorProposalLine` draft-staging tables — deliberately *not* built on `routers/ai.py`/`ollama.py` (no persisted review state existed there to build on), using deterministic sibling-region inheritance + priced-history correlation instead of an LLM.
