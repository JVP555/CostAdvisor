...
- 🔴 Follow-ups: base-price anchors per combo (drop has no prices — coverage `base_price` stays NULL; needed for money-denominated should-cost + engine evaluation); expert-review API/UI (schema ready: list `needs_review`, show `review_metadata`, set reviewed_by/at); surface `blocked`-tier combos (7 TiO₂) in pricing UI; MPOB/SunSirs commercial licensing (business)
