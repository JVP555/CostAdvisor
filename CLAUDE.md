...
- 🟢 **In-app theme selector added to the nav** (was previously only reachable via the full Profile page) — new `components/ThemeSelector.jsx`, a `.ca-menu` dropdown positioned between `TeamSelector` and the account menu, same roving-tabindex/click-outside pattern as `TeamSelector.jsx`. Calls the existing `setTheme()` from `AuthContext.jsx` (already does apply + cache + `PUT /auth/me/theme`) — no new API, no duplicated theme logic; the Profile page's full-page picker (`ThemePreview` cards) is untouched and still there for browsing descriptions
