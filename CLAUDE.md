...
- 🟡 **Scrum 70 (part 1)** — Index projection service (per price series, vintaged run). Named dependencies "the forecast storage subtask" and "W3-A" (series layer) do not exist as separate tickets in this repo — built minimally inline: `(commodity_id, region)` is the series grain directly off `IndexValue` (no new `PriceSeries` entity), and forecasts live in a new pair of tables `IndexValue`'s own queries never touch (so the Scrum 22 backward buy-window signal is unaffected by construction, not by an added filter)
  - 🟢 `IndexProjectionRun` / `IndexProjectionPoint` models + migration `ip1a2b3c4d5e` (no RLS — platform-level, mirrors `commodity_indexes`/`index_values`)
  - 🟢 `services/index_projection.py`: pure-Python OLS linear-trend fit with a residual-based prediction interval; explicit flat/degenerate-hold detection (dead-flat recent tail OR whole-window near-zero variance — the OR catches "volatile history, flat recent tail" a single check would miss); explicit no-history result (never `None`/`[]`); re-running always inserts a new vintage, never overwrites
  - 🟢 `POST /api/indexes/{id}/project` (super-admin, same tier as `/scrape-all`), `GET /api/indexes/{id}/projections/latest` (any authenticated user)
  - 🟢 `tasks/project_indexes.py` weekly Celery task, registered in `celeryconfig.py` beat schedule after the scrape jobs
  - 🟢 Tests `tests/test_index_projection.py` (10): fitted/hold/no-history/insufficient-points, re-run creates a new vintage and leaves the first intact, fitted output pinned against a hand-computed OLS fit, endpoint permission gates
  - 🔴 Not yet verified against a live Postgres in this pass — local dev DB auth was broken (`costadvisor` role password mismatch against a freshly-provisioned native Windows Postgres 18 instance) and the fix was not completed live; verified instead via `py_compile` on every changed/new file, a full `app.main` import (226 routes, both new routes present with correct methods/paths), and a clean `npm run build`. Run `cd backend && alembic upgrade head && pytest tests/test_index_projection.py` once DB access is restored
- 🟡 **Scrum 70 (part 2)** — Lock/hold decision off the forward should-cost. Forward should-cost is computed **generically** off each cost model's own `FormulaVersion` (works for hand-built and catalog-linked models alike) rather than requiring a Wave-2 `FormulaRegionCoverage` catalog link
  - 🟢 `get_forward_index_value` (`data_resolver.py`) — team fixed source / explicit override for the future quarter first, then Part 1's latest projection vintage; deliberately never falls through to `scraped_temporal_carry_forward` (that tier exists to stop historical ratios flattening to 1.0; reusing it here would let a flat hold pose as a real forecast)
  - 🟢 `_compute_indexed_cost_forward` / `_compute_advanced_cost_forward` / `calculate_forward_should_cost` (`costing_engine.py`) — siblings of the existing `_compute_indexed_cost(_detailed)`/`_compute_advanced_cost`, not a parameter threaded into them, so the four existing call sites (should-cost/evolution/squeeze/breakdown) are untouched; hard-fails to `insufficient` the instant one commodity-linked component has no forecast, never a fabricated ratio; an all-fixed formula is deterministically `neutral` ("nothing to forecast" ≠ "missing data")
  - 🟢 `GET /portfolio/buy-windows/{cost_model_id}/verdict` + `LockHoldVerdict` schema — additive sibling of the existing `/buy-windows` endpoints; their response shape is untouched
  - 🟢 Regression test `test_buy_windows_unaffected_by_forecast_storage` — proves forecast storage never perturbs the existing backward signal, because forecasts never land in `index_values`
  - 🟢 Tests `tests/test_buy_windows.py` (+7): insufficient-without-projection, horizon/vintage/method echoed, verdict flips across two vintages (upward → `lock`, downward → `hold`), all-fixed-formula → `neutral`, permission gates
  - 🟢 `ForecastArea.jsx`'s `FORECAST_BAND` placeholder removed entirely (no relabeled reappearance); `BuyWindows.jsx` gained `LockHoldBadge`/`LOCK_HOLD_SIGNAL`; `ProductDetailArea.jsx` fetches the verdict endpoint and renders both badges side by side
  - 🔴 Same DB-verification caveat as Part 1 — `test_buy_windows.py` additions not yet run against a live DB in this pass
...
- 🟢 **SCRUM-70/71 → Scrum 24 (Alerts)** — **already shipped** this session (`services/alerts.py`, subscriptions, index-move/gap/buy-window triggers, email+Slack, history). No action. (Number reused again: a later, unrelated pair of tickets also titled "Scrum 70" — the index projection service and the buy-window lock/hold verdict — landed afterward; see "Scrum 70 (part 1)"/"(part 2)" under Wave 3 — Intelligence & Depth above. Same reuse pattern as SCRUM-74 below — treat by title, not by number.)
