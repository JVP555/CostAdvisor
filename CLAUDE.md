...
- 🔴 **Scrum 9** — Hardened authentication on OAuth 2.0
  ...
  - 🔴 `state` generated per-request, validated, and deleted after use (**re-audited: worse than 🟡** — login discards it entirely, `uri, _state = client.create_authorization_url(...)` in `auth.py`, and the callback never reads/validates a `state` param; the separate calendar OAuth flow does validate state, but the login flow does not)
...
- 🔴 **Scrum 16** — Self-serve onboarding (empty states, example data, guidance to first should-cost vs actual gap)
  ...
  - 🟡 "Load example data" works for a brand-new team and produces a runnable should-cost — CLI/deploy path done (`backend/seed_all.py`: one idempotent command seeding platform data → users/teams → shadow library → demo, in dependency order); the in-app button is still 🔴
...
- 🟢 **Scrum 19** — Automatic gap flagging (dashboard as portfolio triage screen ranked by money at stake)
  ...
  - 🟢 Visual indicator (colour or bar) for gap severity — graded severity bar shipped: `DriftBar` on |gap %| in `MonitorArea.jsx` (the triage screen), plus colour-coded gap% cells + IDX/DRIFT flag badges. `Dashboard.jsx` itself still uses only the colour cell (the graded bar lives in Monitor)
