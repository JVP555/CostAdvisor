      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;border:1px solid #e4e4e7">
        ...
        <tr><td style="padding:32px">
          ...
          <p style="margin:0 0 24px;color:#71717a;font-size:14px">
            <strong style="color:#18181b">{invited_by_name or invited_by_email}</strong>
            {f'({invited_by_email})' if invited_by_name else ''}
            has invited you to join <strong style="color:#18181b">{team_name}</strong>
            on CostAdvisor.
          </p>
          ...
          <p style="margin:0 0 24px;color:#52525b;font-size:13px;line-height:1.6">
            Sign in to CostAdvisor to accept or decline this invitation. The invite will expire in 7 days.
          </p>
          <a href="{requests_url}" style="display:inline-block;background:#18181b;color:#fff;
             text-decoration:none;padding:12px 28px;border-radius:6px;font-size:14px;font-weight:600">
            View Invitation
          </a>
        </td></tr>
        <tr><td style="padding:20px 32px;border-top:1px solid #f4f4f5;background:#fafafa">
          <p style="margin:0;font-size:11px;color:#a1a1aa">
            ...
            &copy; CostAdvisor &middot; <a href="https://costadvisor.org" style="color:#a1a1aa">costadvisor.org</a>
          </p>
        </td></tr>
      </table>
    </td></tr>
...
def send_invite_email(
    ...
    """Send a team invite email via Resend. Returns True on success, False if not configured or failed (non-fatal)."""
    settings = get_settings()
    if not settings.resend_api_key:
        logger.warning("RESEND_API_KEY not configured — invite email not sent to %s", to_email)
        return False
    ...
    try:
        ...
        resend.Emails.send({
            "from": f"CostAdvisor <{settings.email_from}>",
            "to": [to_email],
