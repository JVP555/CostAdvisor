...
- **The "Cost forecast" page (`ForecastArea.jsx`)** — the page's old fake, made-up prediction line (a placeholder number, `0.015`, meant to be temporary) was removed entirely, not just hidden. The page today shows:
...
**Frontend changes (plain English, in detail):**
- **The main Dashboard** gained a 4th view option in its Table / Cards / Matrix toggle row: **"Buy Windows."** Clicking it fetches the signal for every product the first time it's opened (not upfront, to avoid slowing down the page for people who never click it) and shows a dedicated table.
- **That table** (`components/BuyWindows.jsx`) lists, per product: the supplier, today's should-cost, the trailing 4-quarter average, the percentage difference between them, a colour-coded signal badge, and an "Evolution" button that jumps straight to that product's price-history chart. A small note above the table explains the rule in plain terms ("Buy now" = at least 3% cheaper than recent average; "Hold / wait" = at least 3% more expensive), and there's a CSV export button. Products that don't have enough price history yet are quietly counted in a footer line ("3 products without enough index history for a signal yet") instead of cluttering the table with blank rows.
...
**Frontend changes (plain English, in detail), all in the cost-model / product-building screen (`CostModelBuilder.jsx`):**
